...
DTNtupleBaseAnalyzer::DTNtupleBaseAnalyzer(const TString &inFileName) : fChain(0) {
  fChain = new TChain("dtNtupleProducer/DTTREE");
  fChain->Add(inFileName);
  Init(fChain);
}

DTNtupleBaseAnalyzer::DTNtupleBaseAnalyzer(const std::vector<TString> &inFileNames) : fChain(0) {
  fChain = new TChain("dtNtupleProducer/DTTREE");
...
void DTNtupleBaseAnalyzer::Init(TChain *tree) {
  ...
  if (USE_MUONS) {
    ...
    fChain->SetBranchAddress(
        "mu_trk_numberOfValidPixelHits", &mu_trk_numberOfValidPixelHits, &b_mu_trk_numberOfValidPixelHits);
    fChain->SetBranchAddress(
        "mu_trk_numberOfValidTrackerLayers", &mu_trk_numberOfValidTrackerLayers, &b_mu_trk_numberOfValidTrackerLayers);
    fChain->SetBranchAddress("mu_trkMu_stationMask", &mu_trkMu_stationMask, &b_mu_trkMu_stationMask);
    fChain->SetBranchAddress(
        "mu_trkMu_numberOfMatchedStations", &mu_trkMu_numberOfMatchedStations, &b_mu_trkMu_numberOfMatchedStations);
    fChain->SetBranchAddress(
        "mu_trkMu_numberOfMatchedRPCLayers", &mu_trkMu_numberOfMatchedRPCLayers, &b_mu_trkMu_numberOfMatchedRPCLayers);
    fChain->SetBranchAddress(
        "mu_staMu_numberOfValidMuonHits", &mu_staMu_numberOfValidMuonHits, &b_mu_staMu_numberOfValidMuonHits);
...
void DTNtupleBaseAnalyzer::LoadObjs() {
  ...
  digiObjs["Ph1"] = DTNtupleDigi(
      digi_nDigis, digi_wheel, digi_sector, digi_station, digi_superLayer, digi_layer, digi_wire, digi_time);
