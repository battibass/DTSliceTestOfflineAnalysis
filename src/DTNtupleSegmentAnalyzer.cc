...
DTNtupleSegmentAnalyzer::DTNtupleSegmentAnalyzer(const TString &inFileName, const TString &outFileName)
...
DTNtupleSegmentAnalyzer::DTNtupleSegmentAnalyzer(const std::vector<TString> &inFileNames, const TString &outFileName)
...
void DTNtupleSegmentAnalyzer::preLoop() {
  ...
  for (const auto &tag : m_tags) {
    std::string deadFileNameRun = m_baseOutFolder + "/segment/dead_channels/deadChannelList_" + tag;

    std::string deadFileName = "config/deadChannelList_" + tag;

    std::cout << "[DTNtupleSegmentAnalyzer] Info will be saved in " << deadFileNameRun << " and in " << deadFileName
              << " (for future use)" << std::endl;
  ...
  for (Long64_t jentry = 0; jentry < nEntries; ++jentry) {
    ...
    if (jentry % 1000 == 0)
      std::cout << std::setprecision(2) << "[DTNtupleSegmentAnalyzer] : Pre-Loop event " << jentry << " "
                << jentry / float(nEntries) * 100. << "%\r" << std::flush;
  ...
  for (const auto &tag : m_tags) {
        ...
        for (int iLay = 1; iLay <= 4; ++iLay) {
          for (int iW = 1; iW <= nWiresLay; ++iW) {
            ...
            if (hOccupancyBySl[tag][iSt][iSL]->GetBinContent(iW, iLay) == 0) {
              ...
              wireString << nWiresFile << " " << iSt << " " << iSL << " " << iLay << " " << iW << std::endl;
    ...
    std::cout << "[DTNtupleSegmentAnalyzer] : " << nWiresFile << " dead wires out of " << nWiresTot << std::endl;

    std::stringstream trailingString;
    trailingString << 0 << " " << 0 << " " << 0 << " " << 0 << " " << 0 << " " << 0 << " " << 0 << " " << 0 << " "
                   << std::endl
                   << nWiresFile << " dead wires out of " << nWiresTot << std::endl;
...
void DTNtupleSegmentAnalyzer::Loop() {
  ...
  for (Long64_t jentry = 0; jentry < nEntries; jentry++) {
    ...
    if (jentry % 100 == 0)
      std::cout << "[DTNtupleSegmentAnalyzer::Loop] processed : " << jentry << " entries\r" << std::flush;
...
void DTNtupleSegmentAnalyzer::book() {
  ...
  for (const auto &typeTag : typeTags) {
    ...
    m_plots[hName] = new TH1F(hName, "# of segments;# of segments;entries", 51, -0.5, 50.5);
  }

  for (int st = 1; st < 5; st++) {
    auto hName = Form("NSegPh1VsPh2_st%d", st);
    m_plots[hName] =
        new TH2F(hName, "# segment ph1 vs ph2;# seg (phase-1); # seg (phase-2)", 10, -0.5, 9.5, 10, -0.5, 9.5);

    hName = Form("NHitsPhiPh1VsPh2_st%d", st);
    m_plots[hName] = new TH2F(
        hName, "# phi hits ph1 vs ph2;# phi hits (phase-1); # phi hits (phase-2)", 10, -0.5, 9.5, 10, -0.5, 9.5);

    hName = Form("NPh2Ph1HitsDiffPhiVsX_st%d", st);
    m_plots[hName] =
        new TH2F(hName, "# ph2 - ph1 phi hits vs x;local x (cm); # ph2 - ph1 phi hits", 20, -200., 200., 17, -8.5, 8.5);

    hName = Form("EffPhiPh1VsPh2_st%d", st);
    m_plots[hName] =
        new TH2F(hName, "eff ph1 vs ph2;# efficiency (phase-1); efficiency (phase-2)", 101, 0.9, 1.01, 101, 0.9, 1.01);
  }

  for (const auto &typeTag : typeTags) {
    auto hName = Form("%seffPhi_station", typeTag.c_str());
    m_effs[hName] = new TEfficiency(hName, "#phi efficiency per station;station;eff.", 4, 0.5, 4.5);

    hName = Form("%seffTheta_station", typeTag.c_str());
    m_effs[hName] = new TEfficiency(hName, "#theta efficiency per station;station;eff.", 4, 0.5, 4.5);

    hName = Form("%shitResZ", typeTag.c_str());
    m_plots[hName] = new TH1F(hName, "Hit residual z view;|d_hit|-|d_extr|(cm)", 250, -1., 1.);

    hName = Form("%shitResX", typeTag.c_str());
    m_plots[hName] = new TH1F(hName, "Hit residual X view;|d_hit|-|d_extr|(cm)", 250, -.8, .8);

    hName = Form("%shitResX_vsWireDist", typeTag.c_str());
    m_2Dplots[hName] = new TH2F(hName, "Hit residual X view;dist wire;|d_hit|-|d_extr|(cm)", 100, 0, 2.5, 200, -.8, .8);

    hName = Form("%shitResZ_vsWireDist", typeTag.c_str());
    m_2Dplots[hName] = new TH2F(hName, "Hit residual Z view;dist wire;|d_hit|-|d_extr|(cm)", 100, 0, 2.5, 200, -.8, .8);

    for (int st = 1; st < 5; st++) {
      hName = Form("%shPhiNhits_st%d", typeTag.c_str(), st);
      m_plots[hName] = new TH1F(hName, "# hits;# hits; entries", 8, 0.5, 8.5);

      hName = Form("%shNhits_st%d", typeTag.c_str(), st);
      m_plots[hName] = new TH1F(hName, "# hits;# hits; entries", 12, 0.5, 12.5);

      hName = Form("%shT0_st%d", typeTag.c_str(), st);
      m_plots[hName] = new TH1F(hName, "Segment t0;t0 (ns); entries", 200, -100., 100.);

      hName = Form("%shT0VsX_st%d", typeTag.c_str(), st);
      m_plots[hName] = new TProfile(hName, "Segment t0 vs local x;local x (cm);t0 (ns)", 20, -200., 200., -100., 100.);

      hName = Form("%shPhiProbChi2_st%d", typeTag.c_str(), st);
      m_plots[hName] = new TH1F(hName, "Segment #phi prob(#chi^2); prob(#chi^2); entries", 100, 0., 1.);

      hName = Form("%shPhiChi2_st%d", typeTag.c_str(), st);
      m_plots[hName] = new TH1F(hName, "Segment #phi #chi^2;  #chi^2/ndof; entries", 100, 0., 50.);

      if (st < 4) {
        hName = Form("%shThetaNhits_st%d", typeTag.c_str(), st);
        m_plots[hName] = new TH1F(hName, "# hits;# hits; entries", 4, 0.5, 4.5);

        hName = Form("%shThetaProbChi2_st%d", typeTag.c_str(), st);
        m_plots[hName] = new TH1F(hName, "Segment #theta Prob. #chi^2; prob(#chi^2); entries", 100, 0., 1.);

        hName = Form("%shThetaChi2_st%d", typeTag.c_str(), st);
        m_plots[hName] = new TH1F(hName, "Segment #theta #chi^2; #chi^2/ndof; entries", 100, 0., 50.);
      }

      hName = Form("%shResPerSl_st%d", typeTag.c_str(), st);
      m_plots[hName] = new TH1F(hName, "Residual per Sl; Sl ; Residual [#mu m]", 3, 0.5, 3.5);

      hName = Form("%seffByWire_st%d", typeTag.c_str(), st);
      m_effs[hName] =
          new TEfficiency(hName, "Wire by wire efficiency; wire; layer / superlayer", 100, 0.5, 100.5, 12, 0.5, 12.5);

      hName = Form("%soccByWire_st%d", typeTag.c_str(), st);
      m_plots[hName] =
          new TH2F(hName, "Wire by wire occpuancy; wire; layer / superlayer", 100, 0.5, 100.5, 12, 0.5, 12.5);
    ...
    for (int st = 1; st < 5; st++) {
      for (int sl = 1; sl < 4; sl++) {
        if (sl == 2 && st != 4) {
          ...
          m_plots[hName] = new TH1F(hName, "Hit residual #theta view;|d_hit|-|d_extr|(cm)", 250, -.8, .8);

          hName = Form("%shitResZvsWireDist_st%d", typeTag.c_str(), st);
          m_2Dplots[hName] =
              new TH2F(hName, "Hit residual #theta view;|d_wire|[cm];|d_hit|-|d_extr|(cm)", 100, 0, 2.5, 250, -.8, .8);
        } else {
          hName = Form("%shitResX_st%d_sl%d", typeTag.c_str(), st, sl);
          m_plots[hName] = new TH1F(hName, "Hit residual #phi view;|d_hit|-|d_extr|(cm)", 250, -.8, .8);

          hName = Form("%shitResXvsWireDist_st%d_sl%d", typeTag.c_str(), st, sl);
          m_2Dplots[hName] =
              new TH2F(hName, "Hit residual #phi view;|d_wire|[cm];|d_hit|-|d_extr|(cm)", 100, 0, 2.5, 250, -.8, .8);
        }
        ...
        for (int l = 1; l < 5; l++) {
          if (sl == 2 && st != 4) {
            ...
            m_2Dplots[hName] = new TH2F(
                hName, "Hit residual #theta view;|d_wire|[cm];|d_hit|-|d_extr|(cm)", 100, 0, 2.5, 250, -.8, .8);
          } else {
...
void DTNtupleSegmentAnalyzer::endJob() {
  for (const auto &tag : m_tags) {
    for (int st = 1; st < 5; ++st) {
      std::string hName = Form("%seffByWire_st%d", tag.c_str(), st);
      ...
      for (int sl = 1; sl < 4; ++sl) {
        if (sl == 2 && st != 4) {
          std::string hName = Form("%shitResZ_st%d", tag.c_str(), st);
          ...
          TF1 *fTheta = new TF1("fTheta", "gaus", center - 1.2 * sigma, center + 1.2 * sigma);

          m_plots[hName]->Fit("fTheta", "RMQ");
          ...
          std::string hName = Form("%shitResX_st%d_sl%d", tag.c_str(), st, sl);
          ...
          TF1 *fPhi = new TF1("fPhi", "gaus", center - 1.2 * sigma, center + 1.2 * sigma);

          m_plots[hName]->Fit("fPhi", "RMQ");
  ...
  for (int st = 1; st < 5; ++st) {
    std::string hNamePh1 = Form("Ph1effByWire_st%d", st);
    std::string hNamePh2 = Form("Ph2effByWire_st%d", st);
...
void DTNtupleSegmentAnalyzer::baseAnalysis() {
  ...
  for (uint iSeg = 0; iSeg < seg_nSegments; ++iSeg) {
    if (seg_sector->at(iSeg) != 12 || seg_wheel->at(iSeg) != 2)
      continue;
    //Chi2 cut if needed
    ...
    if (seg_hasPhi->at(iSeg)) {
      ...
      if (seg_phi_nHits->at(iSeg) == 8)
        m_plots[Form("Ph1hPhiProbChi2_st%d", seg_station->at(iSeg))]->Fill(
            TMath::Prob(seg_phi_normChi2->at(iSeg) * 6, 6));
    ...
    if (seg_hasZed->at(iSeg)) {
      ...
      if (seg_z_nHits->at(iSeg) == 4)
        m_plots[Form("Ph1hThetaProbChi2_st%d", seg_station->at(iSeg))]->Fill(
            TMath::Prob(seg_z_normChi2->at(iSeg) * 2, 2));
  ...
  for (uint iSeg = 0; iSeg < ph2Seg_nSegments; ++iSeg) {
    if (ph2Seg_sector->at(iSeg) != 12 || ph2Seg_wheel->at(iSeg) != 2)
      continue;
    ...
    if (ph2Seg_hasPhi->at(iSeg)) {
      ...
      if (ph2Seg_phi_nHits->at(iSeg) == 8)
        m_plots[Form("Ph2hPhiProbChi2_st%d", ph2Seg_station->at(iSeg))]->Fill(
            TMath::Prob(ph2Seg_phi_normChi2->at(iSeg) * 6, 6));
    ...
    if (ph2Seg_hasZed->at(iSeg)) {
      ...
      if (ph2Seg_z_nHits->at(iSeg) == 4)
        m_plots[Form("Ph2hThetaProbChi2_st%d", ph2Seg_station->at(iSeg))]->Fill(
            TMath::Prob(ph2Seg_z_normChi2->at(iSeg) * 2, 2));
...
void DTNtupleSegmentAnalyzer::measureEfficiency(std::string Tag) {
  ...
  for (uint iSeg = 0; iSeg < (*seg.nSegments); iSeg++) {
    ...
    if (nHitsPhi < 7)
      continue;
    if (!seg.hasPhi->at(iSeg))
      continue;
    if (seg.sector->at(iSeg) != 12 || seg.wheel->at(iSeg) != 2)
      continue;
    if (fabs(seg.dirLoc_x->at(iSeg)) > 0.7)
      continue;
      ...
      for (int lay = 1; lay < 5; ++lay) {
        ...
        if (!hasFired) {
          for (uint iDigi = 0; iDigi < (*digi.nDigis); ++iDigi) {
            if (digi.wheel->at(iDigi) != 2)
              continue;
            if (digi.sector->at(iDigi) != 12)
              continue;
            if (digi.station->at(iDigi) != station)
              continue;
            if (digi.superLayer->at(iDigi) != sl)
              continue;
            if (digi.layer->at(iDigi) != lay)
              continue;

            float time = digi.time->at(iDigi);
            if (time < m_timeBoxMin.at(Tag) + 750. || time > m_timeBoxMin.at(Tag) + 1650.)
              continue;
...
void DTNtupleSegmentAnalyzer::computeResidual(std::string Tag) {
  DTNtupleSegment &seg = segmentObjs[Tag];
  ...
  for (std::size_t iSeg = 0; iSeg < (*seg.nSegments); ++iSeg) {
    ...
    if (seg.hasPhi->at(iSeg) && segPhiNHits == 8) {
      if (seg.phi_normChi2->at(iSeg) > 3.84)
        continue;
    ...
    if (seg.hasZed->at(iSeg) && segZNHits == 4) {
      if (seg.z_normChi2->at(iSeg) > 3.84)
        continue;
