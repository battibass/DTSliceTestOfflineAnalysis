...
DTNtupleTriggerAnalyzer::DTNtupleTriggerAnalyzer(const TString& inFileName, const TString& outFileName)
  ...
  phi_offset =
      0.525;  // offset between ph2 primitive phi and seg_posGlb_phi of the segment (to be added to phi of the segment)
...
DTNtupleTriggerAnalyzer::DTNtupleTriggerAnalyzer(const std::vector<TString>& inFileNames, const TString& outFileName)
    : DTNtupleBaseAnalyzer(inFileNames), m_outFile(outFileName, "RECREATE") {
  // Configuration parameters to switch on / off
  // different parts of the analyses
  m_twinMux = true;
  m_ph2TpgPhiHw = true;
...
void DTNtupleTriggerAnalyzer::Loop() {
  ...
  for (Long64_t jentry = 0; jentry < nentries; jentry++) {
    ...
    if (jentry % 100 == 0)
      std::cout << "[DTNtupleTriggerAnalyzer::Loop] processed : " << jentry << " entries\r" << std::flush;

    if (jentry > nMaxEvents)
      break;  // set maximum number of processed events
...
void DTNtupleTriggerAnalyzer::book() {
  ...
  for (Int_t iCh = 1; iCh < 5; ++iCh) {
    ...
    if (m_ph2TpgPhiHw && m_segments) {  //*** ph2TpgPhiHw and segments must be "true"
      ...
      m_effs[hName] =
          new TEfficiency(hName.c_str(), "trigger efficiency AnyBX vs t0; t0; primitive effic.", 20, -100., 100.);

      // //***PLOTTED IN JSON
      hName = "trigeff_ph2TpgPhiHw_qual_8_AnyBX_vs_t0_" + iChTag.str();
      m_effs[hName] = new TEfficiency(
          hName.c_str(), "trigger efficiency AnyBX qual 8 vs t0; t0; primitive effic.", 20, -100., 100.);

      //***PLOTTED IN JSON
      hName = "trigeff_ph2TpgPhiHw_qual_6_or_7_AnyBX_vs_t0_" + iChTag.str();
      m_effs[hName] = new TEfficiency(
          hName.c_str(), "trigger efficiency AnyBX qual 8 or 8 vs t0; t0; primitive effic.", 20, -100., 100.);

      //***PLOTTED IN JSON
      hName = "trigeff_ph2TpgPhiHw_qual_6_to_8_AnyBX_vs_t0_" + iChTag.str();
      m_effs[hName] = new TEfficiency(
          hName.c_str(), "trigger efficiency AnyBX qual 6 to 9 vs t0; t0; primitive effic.", 20, -100., 100.);

      //***PLOTTED IN JSON
      hName = "trigeff_ph2TpgPhiHw_qual_3_AnyBX_vs_t0_" + iChTag.str();
      m_effs[hName] = new TEfficiency(
          hName.c_str(), "trigger efficiency AnyBX qual 3 vs t0; t0; primitive effic.", 20, -100., 100.);

      //***PLOTTED IN JSON
      hName = "trigeff_ph2TpgPhiHw_qual_1_AnyBX_vs_t0_" + iChTag.str();
      m_effs[hName] = new TEfficiency(
          hName.c_str(), "trigger efficiency AnyBX qual 1 vs t0; t0; primitive effic.", 20, -100., 100.);

      hName = "trigeff_ph2TpgPhiHw_AnyBX_vs_philoc_" + iChTag.str();
      m_effs[hName] =
          new TEfficiency(hName.c_str(), "trigger efficiency AnyBX vs philoc; philoc; primitive effic", 40, -1., 1.);

      hName = "trigeff_ph2TpgPhiHw_qual_8_AnyBX_vs_philoc_" + iChTag.str();
      m_effs[hName] = new TEfficiency(
          hName.c_str(), "trigger efficiency AnyBX qual 8 vs philoc; philoc; primitive effic", 40, -1., 1.);

      hName = "trigeff_ph2TpgPhiHw_qual_6_or_7_AnyBX_vs_philoc_" + iChTag.str();
      m_effs[hName] = new TEfficiency(
          hName.c_str(), "trigger efficiency AnyBX qual 6 or 7 vs philoc; philoc; primitive effic", 40, -1., 1.);

      hName = "trigeff_ph2TpgPhiHw_qual_6_to_8_AnyBX_vs_philoc_" + iChTag.str();
      m_effs[hName] = new TEfficiency(
          hName.c_str(), "trigger efficiency AnyBX qual 6 to 8 vs philoc; philoc; primitive effic", 40, -1., 1.);

      hName = "trigeff_ph2TpgPhiHw_qual_3_AnyBX_vs_philoc_" + iChTag.str();
      m_effs[hName] = new TEfficiency(
          hName.c_str(), "trigger efficiency AnyBX qual 3 vs philoc; philoc; primitive effic", 40, -1., 1.);

      hName = "trigeff_ph2TpgPhiHw_qual_1_AnyBX_vs_philoc_" + iChTag.str();
      m_effs[hName] = new TEfficiency(
          hName.c_str(), "trigger efficiency AnyBX qual 1 vs philoc; philoc; primitive effic", 40, -1., 1.);

      hName = "trigeff_ph2TpgPhiHw_AnyBX_vs_seg_dirLoc_x_" + iChTag.str();
      m_effs[hName] = new TEfficiency(
          hName.c_str(), "trigger efficiency AnyBX vs seg_dirLoc_x; seg_dirLoc_x; primitive effic", 40, -1.2, 1.2);
      ...
      m_effs[hName] =
          new TEfficiency(hName.c_str(), "trigger efficiency AnyBX vs nHits; nHits; primitive effic", 11, -0.5, 10.5);

      //***PLOTTED IN JSON
      hName = "trigeff_ph2TpgPhiHw_qual_8_AnyBX_vs_nHits_" + iChTag.str();
      m_effs[hName] = new TEfficiency(
          hName.c_str(), "trigger efficiency AnyBX qual 8 vs nHits; nHits; primitive effic", 11, -0.5, 10.5);

      //***PLOTTED IN JSON
      hName = "trigeff_ph2TpgPhiHw_qual_6_or_7_AnyBX_vs_nHits_" + iChTag.str();
      m_effs[hName] = new TEfficiency(
          hName.c_str(), "trigger efficiency AnyBX qual 6 or 7 vs nHits; nHits; primitive effic", 11, -0.5, 10.5);

      //***PLOTTED IN JSON
      hName = "trigeff_ph2TpgPhiHw_qual_6_to_8_AnyBX_vs_nHits_" + iChTag.str();
      m_effs[hName] = new TEfficiency(
          hName.c_str(), "trigger efficiency AnyBX qual 6 to 8 vs nHits; nHits; primitive effic", 11, -0.5, 10.5);

      //***PLOTTED IN JSON
      hName = "trigeff_ph2TpgPhiHw_qual_3_AnyBX_vs_nHits_" + iChTag.str();
      m_effs[hName] = new TEfficiency(
          hName.c_str(), "trigger efficiency AnyBX qual 3 vs nHits; nHits; primitive effic", 11, -0.5, 10.5);

      //***PLOTTED IN JSON
      hName = "trigeff_ph2TpgPhiHw_qual_1_AnyBX_vs_nHits_" + iChTag.str();
      m_effs[hName] = new TEfficiency(
          hName.c_str(), "trigger efficiency AnyBX qual 1 vs nHits; nHits; primitive effic", 11, -0.5, 10.5);

      //***PLOTTED IN JSON
      hName = "trigeff_ph2TpgPhiHw_AnyBX_vs_seg_posLoc_x_" + iChTag.str();  // effic vs posLoc_x
      m_effs[hName] = new TEfficiency(
          hName.c_str(), "trigger efficiency AnyBX vs seg_posLoc_x; posLoc_x; primitive effic", 50, -200., 200.);

      //***PLOTTED IN JSON
      hName = "trigeff_ph2TpgPhiHw_qual_8_AnyBX_vs_seg_posLoc_x_" + iChTag.str();  // effic vs posLoc_x
      m_effs[hName] = new TEfficiency(
          hName.c_str(), "trigger efficiency AnyBX qual_8 vs seg_posLoc_x; posLoc_x; primitive effic", 50, -200., 200.);
      //***PLOTTED IN JSON
      ...
      m_effs[hName] = new TEfficiency(
          hName.c_str(), "trigger efficiency AnyBX qual 3 vs seg_posLoc_x; posLoc_x; primitive effic", 50, -200., 200.);
      //***PLOTTED IN JSON
      hName = "trigeff_ph2TpgPhiHw_qual_1_AnyBX_vs_seg_posLoc_x_" + iChTag.str();  // effic vs posLoc_x
      m_effs[hName] = new TEfficiency(
          hName.c_str(), "trigger efficiency AnyBX qual 1 vs seg_posLoc_x; posLoc_x; primitive effic", 50, -200., 200.);

      hName = "trigeff_ph2TpgPhiHw_AnyBX_vs_dirGlb_phi_" + iChTag.str();
      m_effs[hName] = new TEfficiency(
          hName.c_str(), "trigger efficiency AnyBX vs dirGlb_phi; dirGlb_phi; primitive effic", 30, -2., 1.);

      hName = "trigeff_ph2TpgPhiHw_AnyBX_qual_8_vs_dirGlb_phi_" + iChTag.str();
      m_effs[hName] = new TEfficiency(
          hName.c_str(), "trigger efficiency AnyBX qual 8 vs dirGlb_phi; dirGlb_phi; primitive effic", 30, -2., 1.);

      hName = "trigeff_ph2TpgPhiHw_AnyBX_qual_6_or_7_vs_dirGlb_phi_" + iChTag.str();
      m_effs[hName] = new TEfficiency(
          hName.c_str(), "trigger efficiency AnyBX qual 6 or 7 vs dirGlb_phi; dirGlb_phi; primitive effic", 30, -2., 1.);

      hName = "trigeff_ph2TpgPhiHw_AnyBX_qual_6_to_8_vs_dirGlb_phi_" + iChTag.str();
      m_effs[hName] = new TEfficiency(
          hName.c_str(), "trigger efficiency qual 6 to 8 AnyBX vs dirGlb_phi; dirGlb_phi; primitive effic", 30, -2., 1.);

      hName = "trigeff_ph2TpgPhiHw_AnyBX_qual_3_vs_dirGlb_phi_" + iChTag.str();
      m_effs[hName] = new TEfficiency(
          hName.c_str(), "trigger efficiency AnyBX qual 3 vs dirGlb_phi; dirGlb_phi; primitive effic", 30, -2., 1.);

      hName = "trigeff_ph2TpgPhiHw_AnyBX_qual_1_vs_dirGlb_phi_" + iChTag.str();
      m_effs[hName] = new TEfficiency(
          hName.c_str(), "trigger efficiency AnyBX qual 1 vs dirGlb_phi; dirGlb_phi; primitive effic", 30, -2., 1.);

      //***PLOTTED IN JSON
      hName = "trigeff_ph2TpgPhiHw_AnyBX_vs_posGlb_phi_" + iChTag.str();
      m_effs[hName] = new TEfficiency(
          hName.c_str(), "trigger efficiency AnyBX vs posGlb_phi; posGlb_phi; ph2TpgPhiHw effic", 50, -1.0, -0.1);

      //***PLOTTED IN JSON
      hName = "trigeff_ph2TpgPhiHw_AnyBX_qual_8_vs_posGlb_phi_" + iChTag.str();
      m_effs[hName] = new TEfficiency(
          hName.c_str(), "trigger efficiency AnyBX qual 8 vs posGlb_phi; posGlb_phi; ph2TpgPhiHw effic", 50, -1.0, -0.1);
      ...
      m_effs[hName] = new TEfficiency(
          hName.c_str(), "trigger efficiency AnyBX qual 3 vs posGlb_phi; posGlb_phi; ph2TpgPhiHw effic", 50, -1.0, -0.1);

      //***PLOTTED IN JSON
      hName = "trigeff_ph2TpgPhiHw_AnyBX_qual_1_vs_posGlb_phi_" + iChTag.str();
      m_effs[hName] = new TEfficiency(
          hName.c_str(), "trigger efficiency AnyBX qual 1 vs posGlb_phi; posGlb_phi; ph2TpgPhiHw effic", 50, -1.0, -0.1);

      //

      hName =
          "trigeff_ph2TpgPhiHw_AnyBX_vs_dirGlb_phi_posGlb_phi_" + iChTag.str();  // vs posGlb_phi (x) and dirGlb_phi (y)
      m_effs[hName] = new TEfficiency(
          hName.c_str(),
      ...
      m_effs[hName] = new TEfficiency(
          hName.c_str(), "trigger efficiency AnyBX vs normChi2; normChi2; ph2TpgPhiHw effic", 60, 0., 20.);
      ...
      m_effs[hName] =
          new TEfficiency(hName.c_str(), "trigger efficiency BXOK vs t0; t0; primitive effic.", 20, -100., 100.);

      hName = "trigeff_ph2TpgPhiHw_BXOK_vs_philoc_" + iChTag.str();
      m_effs[hName] =
          new TEfficiency(hName.c_str(), "trigger efficiency BXOK vs philoc; philoc; primitive effic", 40, -80., 80.);

      hName = "trigeff_ph2TpgPhiHw_BXOK_vs_nHits_" + iChTag.str();
      m_effs[hName] =
          new TEfficiency(hName.c_str(), "trigger efficiency BXOK vs nHits; nHits; primitive effic", 11, -0.5, 10.5);

      hName = "trigeff_ph2TpgPhiHw_BXOK_vs_dirGlb_phi_" + iChTag.str();
      m_effs[hName] = new TEfficiency(
          hName.c_str(), "trigger efficiency BXOK vs dirGlb_phi; dirGlb_phi; primitive effic", 30, -2., 1.);

      hName = "trigeff_ph2TpgPhiHw_BXOK_vs_posGlb_phi_" + iChTag.str();
      m_effs[hName] = new TEfficiency(
          hName.c_str(), "trigger efficiency BXOK vs posGlb_phi; posGlb_phi; ph2TpgPhiHw effic", 50, -1.0, -0.1);

      hName = "trigeff_ph2TpgPhiHw_BXOK_vs_normChi2_" + iChTag.str();
      m_effs[hName] = new TEfficiency(
          hName.c_str(), "trigger efficiency BXOK vs normChi2; normChi2; ph2TpgPhiHw effic", 60, 0., 20.);
    }
    ...
    if (m_ph2TpgPhiEmuAm && m_segments) {  //*** ph2TpgEmuAm and segments must be "true"
      ...
      m_effs[hName] =
          new TEfficiency(hName.c_str(), "trigger efficiency AnyBX vs philoc; philoc; primitive effic", 40, -80., 80.);
      ...
      m_effs[hName] = new TEfficiency(
          hName.c_str(), "trigger efficiency AnyBX vs posGlb_phi; posGlb_phi; ph2TpgPhiEmuAm effic", 50, -1.0, -0.1);

      hName = "trigeff_ph2TpgPhiEmuAm_AnyBX_vs_normChi2_" + iChTag.str();
      m_effs[hName] = new TEfficiency(
          hName.c_str(), "trigger efficiency AnyBX vs normChi2; normChi2; ph2TpgPhiEmuAm effic", 60, 0., 20.);
      ...
      m_effs[hName] = new TEfficiency(
          hName.c_str(), "trigger efficiency BXOK vs posGlb_phi; posGlb_phi; ph2TpgPhiEmuAm effic", 50, -1.0, -0.1);

      hName = "trigeff_ph2TpgPhiEmuAm_BXOK_vs_normChi2_" + iChTag.str();
      m_effs[hName] = new TEfficiency(
          hName.c_str(), "trigger efficiency BXOK vs normChi2; normChi2; ph2TpgPhiEmuAm effic", 60, 0., 20.);
    }
    ...
    if (m_twinMux && m_segments) {  //*** TwinMux and segments must be "true"
      ...
      m_effs[hName] = new TEfficiency(
          hName.c_str(), "trigger efficiency AnyBX vs posGlb_phi; posGlb_phi; TwinMuxOut effic", 50, -1.0, -0.1);
      ...
      m_effs[hName] = new TEfficiency(
          hName.c_str(), "trigger efficiency AnyBX vs normChi2; normChi2; TwinMuxOut effic", 60, 0., 20.);
      ...
      m_effs[hName] = new TEfficiency(
          hName.c_str(), "trigger efficiency BXOK vs posGlb_phi; posGlb_phi; TwinMuxOut effic", 50, -1.0, -0.1);

      hName = "trigeff_TwinMuxOut_BXOK_vs_normChi2_" + iChTag.str();
      m_effs[hName] = new TEfficiency(
          hName.c_str(), "trigger efficiency BXOK vs normChi2; normChi2; TwinMuxOut effic", 60, 0., 20.);
    }
    ...
    if (m_ph2TpgPhiHw && m_segments) {  //*** ph2TpgPhiHw and segments must be "true"
      ...
      hName = "ph2TpgPhiHw_posLoc_x_qual_8_minus_seg_posLoc_x_" +
              iChTag.str();  //  residuals ph2 posLoc_x minus seg posLoc_x
      m_plots[hName] = new TH1F(hName.c_str(),
      ...
      hName = "ph2TpgPhiHw_posLoc_x_qual_6_or_7_minus_seg_posLoc_x_" +
              iChTag.str();  //  residuals ph2 posLoc_x minus seg posLoc_x
      m_plots[hName] =
      ...
      hName = "ph2TpgPhiHw_posLoc_x_qual_6_to_8_minus_seg_posLoc_x_" +
              iChTag.str();  //  residuals ph2 posLoc_x minus seg posLoc_x
      m_plots[hName] =
          new TH1F(hName.c_str(),
      ...
      hName = "ph2TpgPhiHw_posLoc_x_qual_3_minus_seg_posLoc_x_" +
              iChTag.str();  //  residuals ph2 posLoc_x minus seg posLoc_x
      m_plots[hName] = new TH1F(hName.c_str(),
      ...
      hName = "ph2TpgPhiHw_posLoc_x_qual_1_minus_seg_posLoc_x_" +
              iChTag.str();  //  residuals ph2 posLoc_x minus seg posLoc_x
      m_plots[hName] = new TH1F(hName.c_str(),
      ...
      m_plots[hName] =
          new TH2F(hName.c_str(), "Phase2Hw t0 vs segment_t0; Segment t0; Phase2Hw t0", 60, -120., 120., 60, -120, 120);
      //***PLOTTED IN JSON
      hName = "Phase2Hw_t0_minus_segment_t0_" + iChTag.str();  // Phase2 prim tminus segment t0
      m_plots[hName] =
          new TH1F(hName.c_str(), "Phase2Hw t0 - segment_t0; Phase 2 trig t0 - Segment t0; Entries", 120, -120., 120.);

      //***PLOTTED IN JSON --
      hName = "Phase2Hw_t0_minus_segment_t0_qual_8_" + iChTag.str();  // Phase2 prim tminus segment t0  qual_8
      m_plots[hName] = new TH1F(
          hName.c_str(), "Phase2Hw t0 - segment_t0 for qual 8; Phase 2 trig t0 - Segment t0; Entries", 120, -120., 120.);
      ...
      m_plots[hName] = new TH1F(
          hName.c_str(), "Phase2Hw t0 - segment_t0 for qual 3; Phase 2 trig t0 - Segment t0; Entries", 120, -120., 120.);

      //***PLOTTED IN JSON --
      hName = "Phase2Hw_t0_minus_segment_t0_qual_1_" + iChTag.str();  // Phase2 prim tminus segment t0  qual 4 or 3
      m_plots[hName] = new TH1F(
          hName.c_str(), "Phase2Hw t0 - segment_t0 for qual 1; Phase 2 trig t0 - Segment t0; Entries", 120, -120., 120.);
    ...
    if (m_twinMux) {  //*** TwinMux must be "true"
      ...
      m_plots[hName] = new TH1F(hName.c_str(), "BX;BX;#entries/BX", 11, -5.5, 5.5);

      hName = "quality_" + iChTag.str();  // quality distribution
      m_plots[hName] = new TH1F(hName.c_str(), "quality;quality;#entries/quality", 11, -0.5, 10.5);

      hName = "phi_" + iChTag.str();  // phi distribution
      m_plots[hName] = new TH1F(hName.c_str(), "phi;phi;#entries/phi", 50, -0.9, 0.9);

      hName = "phiB_" + iChTag.str();  // phiB distribution
      m_plots[hName] = new TH1F(hName.c_str(), "phiB;phiB;#entries/phiB", 60, -1.2, 1.2);
      ...
      m_plots[hName] = new TH1F(hName.c_str(), "whenBXOK quality;quality;#entries/quality", 11, -0.5, 10.5);

      hName = "BXOK_phi_" + iChTag.str();  // phi distribution
      m_plots[hName] = new TH1F(hName.c_str(), "whenBXOK phi;phi;#entries/phi", 60, -1.2, 1.2);

      hName = "BXOK_phiB_" + iChTag.str();  // phiB distribution
      m_plots[hName] = new TH1F(hName.c_str(), "whenBXOK phiB;phiB;#entries/phiB", 60, -1.2, 1.2);
    }
    ...
    if (m_ph2TpgPhiHw) {  //*** ph2TpgPhiHw  must be "true"
      ...
      m_plots[hName] = new TH1F(hName.c_str(), "Phase2Hw BX;BX;#entries/Phase2Hw BX", 21, -10.5, 10.5);
      //***PLOTTED IN JSON
      hName = "Phase2Hw_quality_" + iChTag.str();  // quality distribution
      m_plots[hName] = new TH1F(hName.c_str(), "Phase2Hw quality; quality;#entries/quality", 11, -0.5, 10.5);
      //***PLOTTED IN JSON
      hName = "Phase2Hw_phi_" + iChTag.str();  // phi distribution
      m_plots[hName] = new TH1F(hName.c_str(), "Phase2Hw phi;Phase2Hw phi;#entries/Phase2Hw phi", 100, -1., 1.);
      //	 60,-1.2,1.2);
      //***PLOTTED IN JSON
      hName = "Phase2Hw_phiB_" + iChTag.str();  // phiB distribution
      m_plots[hName] = new TH1F(hName.c_str(), "Phase2 phiB;phiB;#entries/phiB", 100, -1.2, 1.2);

      hName = "Phase2Hw_phi_plus_phiB_" + iChTag.str();  // phase2 trig phi + phiB
      m_plots[hName] = new TH1F(hName.c_str(), "Phase2Hw phi+phiB;phi+phiB;entries/phi+phiB", 50, -1.2, 1.2);

      //***PLOTTED IN JSON
      hName = "Phase2Hw_chi2_" + iChTag.str();  // chi2 distribution
      m_plots[hName] = new TH1F(hName.c_str(), "Phase2Hw chi2;Phase2Hw chi2;#entries/Phase2Hw chi2", 32, 0., 32.);
      //***PLOTTED IN JSON
      hName = "Phase2Hw_t0_" + iChTag.str();  // t0 distribution
      m_plots[hName] = new TH1F(hName.c_str(), "Phase2Hw t0;Phase2Hw t0;#entries/Phase2Hw t0", 60, -120., 120.);
      //***PLOTTED IN JSON
      hName = "Phase2Hw_BX_vs_quality_" + iChTag.str();  // Phase2Hw BX vs quality distribution
      m_plots[hName] = new TH2F(
          hName.c_str(), "Phase2Hw BX vs quality;Phase2Hw quality;Phase2Hw BX", 11, -0.5, 10.5, 21, -10.5, 10.5);

      //
      hName = "Phase2Hw_t0_vs_phi_" + iChTag.str();  // phase2 trig t0 vs phi
      m_plots[hName] =
          new TH2F(hName.c_str(), "Phase2Hw t0 vs phi;Phase2Hw phi;Phase2Hw t0", 50, -0.4, 0.4, 60, -120., 120.);

      //
      hName = "Phase2Hw_t0_vs_phiB_" + iChTag.str();  // phase2 trig t0 vs phiB
      m_plots[hName] =
          new TH2F(hName.c_str(), "Phase2Hw t0 vs phiB;Phase2Hw phiB;Phase2Hw t0", 50, -1.2, 1.2, 60, -120., 120.);

      hName = "Phase2Hw_t0_vs_phi_plus_phiB_" + iChTag.str();  // phase2 trig t0 vs phi + phiB
      m_plots[hName] = new TH2F(
          hName.c_str(), "Phase2Hw t0 vs phi+phiB;Phase2Hw phi+phiB;Phase2Hw t0", 50, -1.2, 1.2, 60, -120., 120.);

      // ------------ now Phase2Hw (Madrid) with BXOK

      hName = "Phase2Hw_BXOK_quality_" + iChTag.str();  // quality distribution
      m_plots[hName] = new TH1F(hName.c_str(), "Phase2Hw BXOK quality; quality;#entries/quality", 11, -0.5, 10.5);

      hName = "Phase2Hw_BXOK_phi_" + iChTag.str();  // phi distribution
      m_plots[hName] = new TH1F(hName.c_str(), "Phase2Hw BXOK phi;Phase2Hw phi;#entries/Phase2Hw phi", 100, -1., 1.);

      hName = "Phase2Hw_BXOK_phiB_" + iChTag.str();  // phiB distribution
      m_plots[hName] = new TH1F(hName.c_str(), "Phase2 BXOK phiB;phiB;#entries/phiB", 100, -1.2, 1.2);

      hName = "Phase2Hw_BXOK_chi2_" + iChTag.str();  // chi2 distribution
      m_plots[hName] = new TH1F(hName.c_str(), "Phase2Hw BXOK chi2;Phase2Hw chi2;#entries/Phase2Hw chi2", 32, 0., 32.);

      hName = "Phase2Hw_BXOK_t0_" + iChTag.str();  // t0 distribution
      m_plots[hName] = new TH1F(hName.c_str(), "Phase2Hw BXOK t0;Phase2Hw t0;#entries/Phase2Hw t0", 60, -120., 120.);
    }
    if (m_ph2TpgPhiHw && m_twinMux) {  //*** ph2TpgPhiHw and TwinMux must be "true"
      ...
      m_plots[hName] =
          new TH2F(hName.c_str(), "Phase2Hw_vs_TMux BX;TMux_BX; Ph2TpgHw BX", 11, -5.5, 5.5, 21, -10.5, 10.5);
      ...
      m_plots[hName] = new TH2F(hName.c_str(),
                                ...
                                10.5);
      //NEW uuu
      hName = "Phase2Hw_minus_TwinMuxOut_quality_vs_TwinMuxOut_quality_" +
              iChTag.str();  // Delta primitive quality vs TMux quality
      m_plots[hName] =
          new TH2F(hName.c_str(),
      ...
      m_plots[hName] = new TH2F(
          hName.c_str(), "Phase2Hw_vs_TMux quality; TMux quality; Ph2TpgHw quality", 11, -0.5, 10.5, 11, -0.5, 10.5);
      //***PLOTTED IN JSON
      hName = "Phase2Hw_vs_TwinMuxOut_phi_" + iChTag.str();  // phi distribution
      m_plots[hName] =
          new TH2F(hName.c_str(), "Phase2Hw_vs_TMux phi;TMux phi; Ph2TpgHw phi", 100, -0.4, 0.3, 100, -0.4, 0.3);
      //	 60,-1.2,1.2);

      //***PLOTTED IN JSON
      hName = "Phase2Hw_vs_TwinMuxOut_phiB_" + iChTag.str();  // phiB distribution
      m_plots[hName] =
          new TH2F(hName.c_str(), "Phase2Hw_vs_TMux phiB; TMux phiB; Ph2TpgHw phiB", 100, -1.2, 1.2, 100, -1.2, 1.2);
    }
    ...
    if (m_ph2TpgPhiHw && m_ph2TpgPhiEmuAm) {  //*** ph2TpgPhiHw  and ph2TpgPhiEmuAm must be "true"
      ...
      m_plots[hName] = new TH2F(hName.c_str(), "Phase2Hw_vs_Emu BX;BX_emu; BX_Hw", 21, -10.5, 10.5, 21, -10.5, 10.5);

      hName = "Phase2Hw_vs_EmuAm_quality_" + iChTag.str();  // quality distribution
      m_plots[hName] =
          new TH2F(hName.c_str(), "Phase2Hw_vs_Emu quality; quality;#entries/quality", 11, -0.5, 10.5, 11, -0.5, 10.5);

      hName = "Phase2Hw_vs_EmuAm_phi_" + iChTag.str();  // phi distribution
      m_plots[hName] =
          new TH2F(hName.c_str(), "Phase2Hw_vs_Emu phi;Phase2Hw phi;#entries/Phase2Hw phi", 100, -1., 1., 100, -1., 1.);
      //	 60,-1.2,1.2);

      hName = "Phase2Hw_vs_EmuAm_phiB_" + iChTag.str();  // phiB distribution
      m_plots[hName] =
          new TH2F(hName.c_str(), "Phase2Hw_vs_Emu phiB;phiB;#entries/phiB", 100, -100, 100, 100, -100, 100);

      hName = "Phase2Hw_vs_EmuAm_chi2_" + iChTag.str();  // chi2 distribution
      m_plots[hName] = new TH2F(
          hName.c_str(), "Phase2Hw_vs_Emu chi2;Phase2Hw chi2;#entries/Phase2Hw chi2", 32, 0., 32., 32, 0., 32.);

      hName = "Phase2Hw_vs_EmuAm_t0_" + iChTag.str();  // t0 distribution
      m_plots[hName] = new TH2F(
          hName.c_str(), "Phase2Hw_vs_Emu t0;Phase2Hw t0;#entries/Phase2Hw t0", 60, -120., 120., 60, -120., 120.);
    }
    ...
    if (m_ph2TpgPhiHw && m_ph2TpgPhiEmuHb) {  //*** ph2TpgPhiHw  and ph2TpgPhiEmuHb must be "true"
      ...
      m_plots[hName] = new TH2F(hName.c_str(), "Phase2Hw_vs_EmuHb BX;BX_emu; BX_Hw", 21, -10.5, 10.5, 20, 10.5, 30.5);

      hName = "Phase2Hw_vs_EmuHb_quality_" + iChTag.str();  // quality distribution
      m_plots[hName] = new TH2F(
          hName.c_str(), "Phase2Hw_vs_EmuHb quality; quality;#entries/quality", 11, -0.5, 10.5, 11, -0.5, 10.5);

      hName = "Phase2Hw_vs_EmuHb_phi_" + iChTag.str();  // phi distribution
      m_plots[hName] = new TH2F(
          hName.c_str(), "Phase2Hw_vs_EmuHb phi;Phase2Hw phi;#entries/Phase2Hw phi", 100, -1., 1., 100, -1., 1.);

      hName = "Phase2Hw_vs_EmuHb_phiB_" + iChTag.str();  // phiB distribution
      m_plots[hName] =
          new TH2F(hName.c_str(), "Phase2Hw_vs_EmuHb phiB;phiB;#entries/phiB", 100, -100, 100, 100, -100, 100);

      hName = "Phase2Hw_vs_EmuHb_chi2_" + iChTag.str();  // chi2 distribution
      m_plots[hName] = new TH2F(
          hName.c_str(), "Phase2Hw_vs_EmuHb chi2;Phase2Hw chi2;#entries/Phase2Hw chi2", 32, 0., 32., 32, 0., 32.);

      hName = "Phase2Hw_vs_EmuHb_t0_" + iChTag.str();  // t0 distribution
      m_plots[hName] = new TH2F(
          hName.c_str(), "Phase2Hw_vs_EmuHb t0;Phase2Hw t0;#entries/Phase2Hw t0", 100, 0., 90000., 100, 0., 90000.);
    }
    ...
    if (m_ph2TpgPhiEmuHb) {
      ...
      m_plots[hName] = new TH1F(hName.c_str(), "Phase2EmuHb BX;BX;#entries/Phase2EmuHb BX", 60, -300.5, -240.5);

      hName = "Phase2EmuHb_quality_" + iChTag.str();  // quality distribution
      m_plots[hName] = new TH1F(hName.c_str(), "Phase2EmuHb quality; quality;#entries/quality", 11, -0.5, 10.5);

      hName = "Phase2EmuHb_phi_" + iChTag.str();  // phi distribution
      m_plots[hName] =
          new TH1F(hName.c_str(), "Phase2EmuHb phi;Phase2EmuHb phi;#entries/Phase2EmuHb phi", 100, -1., 1.);
      //	 60,-1.2,1.2);

      hName = "Phase2EmuHb_phiB_" + iChTag.str();  // phiB distribution
      m_plots[hName] = new TH1F(hName.c_str(), "Phase2 phiB;phiB;#entries/phiB", 100, -100, 100);

      hName = "Phase2EmuHb_chi2_" + iChTag.str();  // chi2 distribution
      m_plots[hName] =
          new TH1F(hName.c_str(), "Phase2EmuHb chi2;Phase2EmuHb chi2;#entries/Phase2EmuHb chi2", 32, 0., 32.);

      hName = "Phase2EmuHb_t0_" + iChTag.str();  // t0 distribution
      m_plots[hName] =
          new TH1F(hName.c_str(), "Phase2EmuHb t0;Phase2EmuHb t0;#entries/Phase2EmuHb t0", 100, 0., 90000.);

      // ------------ now Phase2EmuHb (Pozzobon) with BXOK

      hName = "Phase2EmuHb_BXOK_quality_" + iChTag.str();  // quality distribution
      m_plots[hName] = new TH1F(hName.c_str(), "Phase2EmuHb BXOK quality; quality;#entries/quality", 11, -0.5, 10.5);

      hName = "Phase2EmuHb_BXOK_phi_" + iChTag.str();  // phi distribution
      m_plots[hName] =
          new TH1F(hName.c_str(), "Phase2EmuHb BXOK phi;Phase2EmuHb phi;#entries/Phase2EmuHb phi", 100, -1., 1.);
      //	 60,-1.2,1.2);

      hName = "Phase2EmuHb_BXOK_phiB_" + iChTag.str();  // phiB distribution
      m_plots[hName] = new TH1F(hName.c_str(), "Phase2 BXOK phiB;phiB;#entries/phiB", 100, -100, 100);

      hName = "Phase2EmuHb_BXOK_chi2_" + iChTag.str();  // chi2 distribution
      m_plots[hName] =
          new TH1F(hName.c_str(), "Phase2EmuHb BXOK chi2;Phase2EmuHb chi2;#entries/Phase2EmuHb chi2", 32, 0., 32.);

      hName = "Phase2EmuHb_BXOK_t0_" + iChTag.str();  // t0 distribution
      m_plots[hName] =
          new TH1F(hName.c_str(), "Phase2EmuHb BXOK t0;Phase2EmuHb t0;#entries/Phase2EmuHb t0", 100, 0., 90000.);
    }
    ...
    if (m_ph2TpgPhiEmuAm) {
      ...
      m_plots[hName] = new TH1F(hName.c_str(), "Phase2EmuAm BX;BX;#entries/Phase2EmuAm BX", 60, -300.5, -240.5);

      hName = "Phase2EmuAm_quality_" + iChTag.str();  // quality distribution
      m_plots[hName] = new TH1F(hName.c_str(), "Phase2EmuAm quality; quality;#entries/quality", 11, -0.5, 10.5);

      hName = "Phase2EmuAm_phi_" + iChTag.str();  // phi distribution
      m_plots[hName] =
          new TH1F(hName.c_str(), "Phase2EmuAm phi;Phase2EmuAm phi;#entries/Phase2EmuAm phi", 100, -1., 1.);
      //	 60,-1.2,1.2);
      ...
      m_plots[hName] =
          new TH1F(hName.c_str(), "Phase2EmuAm chi2;Phase2EmuAm chi2;#entries/Phase2EmuAm chi2", 32, 0., 32.);

      hName = "Phase2EmuAm_t0_" + iChTag.str();  // t0 distribution
      m_plots[hName] =
          new TH1F(hName.c_str(), "Phase2EmuAm t0;Phase2EmuAm t0;#entries/Phase2EmuAm t0", 60, -120., 120.);

      // ------------ now Phase2EmuAm (Madrid) with BXOK

      hName = "Phase2EmuAm_BXOK_quality_" + iChTag.str();  // quality distribution
      m_plots[hName] = new TH1F(hName.c_str(), "Phase2EmuAm BXOK quality; quality;#entries/quality", 11, -0.5, 10.5);

      hName = "Phase2EmuAm_BXOK_phi_" + iChTag.str();  // phi distribution
      m_plots[hName] =
          new TH1F(hName.c_str(), "Phase2EmuAm BXOK phi;Phase2EmuAm phi;#entries/Phase2EmuAm phi", 100, -1., 1.);
      //	 60,-1.2,1.2);
      ...
      m_plots[hName] =
          new TH1F(hName.c_str(), "Phase2EmuAm BXOK chi2;Phase2EmuAm chi2;#entries/Phase2EmuAm chi2", 32, 0., 32.);

      hName = "Phase2EmuAm_BXOK_t0_" + iChTag.str();  // t0 distribution
      m_plots[hName] =
          new TH1F(hName.c_str(), "Phase2EmuAm BXOK t0;Phase2EmuAm t0;#entries/Phase2EmuAm t0", 100, 0., 90000.);
    }
  }
}
  ...
  for (Int_t iMB = 1; iMB < 5; ++iMB) {
    ...
    if (m_segments) {
      if (iBestSeg[iMB - 1] < 9999) {  // there is a good segment in the station we are in
        ...
        if (m_ph2TpgPhiHw) {  // ph2TpgPhiHw efficiency at AnyBX
          ...
          hName = "trigeff_ph2TpgPhiHw_qual_6_or_7_AnyBX_vs_t0_" + iChTag.str();
          m_effs[hName]->Fill(
              (iBestTpgPhiHw[iMB - 1] < 9999) && (ph2TpgPhiHw_quality->at(iBestTpgPhiHw[iMB - 1]) == 6 ||
          ...
          hName = "trigeff_ph2TpgPhiHw_AnyBX_vs_dirGlb_phi_posGlb_phi_" +
                  iChTag.str();  // vs posGlb_phi (x) and dirGlb_phi (y)
        ...
        if (m_ph2TpgPhiHw) {  // ph2TpgPhiHw and segment correlation - no 30 deg cut on segmnent inclination
          ...
          if (iBestTpgPhiHw[iMB - 1] < 9999) {
            ...
            m_plots[hName]->Fill(seg_posLoc_x->at(iBestSeg[iMB - 1]),
            ...
            m_plots[hName]->Fill(seg_posGlb_phi->at(iBestSeg[iMB - 1]),
                                 -phi_offset + phi_Ph2_conv * ph2TpgPhiHw_phi->at(iBestTpgPhiHw[iMB - 1]));

            hName = "ph2TpgPhiHw_phi_minus_seg_posGlb_phi_" + iChTag.str();  // rsiduals ph2 phi minus seg posGlb_phi
            m_plots[hName]->Fill(phi_Ph2_conv * ph2TpgPhiHw_phi->at(iBestTpgPhiHw[iMB - 1]) -
                                 (phi_offset + seg_posGlb_phi->at(iBestSeg[iMB - 1])));
            ...
            hName =
                "ph2TpgPhiHw_posLoc_x_minus_seg_posLoc_x_" + iChTag.str();  // residuals ph2 posLoc_x minus seg posLoc_x
            m_plots[hName]->Fill(ph2TpgPhiHw_posLoc_x->at(iBestTpgPhiHw[iMB - 1]) -
                                 seg_posLoc_x->at(iBestSeg[iMB - 1]));
            ...
            if (ph2TpgPhiHw_quality->at(iBestTpgPhiHw[iMB - 1]) == 8) {
              //***PLOTTED IN JSON
              hName =
                  "Phase2Hw_t0_minus_segment_t0_qual_8_" + iChTag.str();  // Phase2 prim t minus segment t0 for qual_8
              m_plots[hName]->Fill(ph2TpgPhiHw_t0->at(iBestTpgPhiHw[iMB - 1]) - seg_phi_t0->at(iBestSeg[iMB - 1]));
            }
            if (ph2TpgPhiHw_quality->at(iBestTpgPhiHw[iMB - 1]) >= 6) {
              //***PLOTTED IN JSON
              hName = "Phase2Hw_t0_minus_segment_t0_qual_6_to_8_" +
                      iChTag.str();  // Phase2 prim t minus segment t0 for qual 6 to 8
              m_plots[hName]->Fill(ph2TpgPhiHw_t0->at(iBestTpgPhiHw[iMB - 1]) - seg_phi_t0->at(iBestSeg[iMB - 1]));
            }
            if (ph2TpgPhiHw_quality->at(iBestTpgPhiHw[iMB - 1]) == 6 ||
                ph2TpgPhiHw_quality->at(iBestTpgPhiHw[iMB - 1]) == 7) {
              //***PLOTTED IN JSON
              hName = "Phase2Hw_t0_minus_segment_t0_qual_6_or_7_" +
                      iChTag.str();  // Phase2 prim t minus segment t0 for qual 6 or 7
              m_plots[hName]->Fill(ph2TpgPhiHw_t0->at(iBestTpgPhiHw[iMB - 1]) - seg_phi_t0->at(iBestSeg[iMB - 1]));
            }
            if (ph2TpgPhiHw_quality->at(iBestTpgPhiHw[iMB - 1]) == 3) {
              //***PLOTTED IN JSON
              hName =
                  "Phase2Hw_t0_minus_segment_t0_qual_3_" + iChTag.str();  // Phase2 prim t minus segment t0 for qual 3
              m_plots[hName]->Fill(ph2TpgPhiHw_t0->at(iBestTpgPhiHw[iMB - 1]) - seg_phi_t0->at(iBestSeg[iMB - 1]));
            }
            if (ph2TpgPhiHw_quality->at(iBestTpgPhiHw[iMB - 1]) == 1) {
              //***PLOTTED IN JSON
              hName =
                  "Phase2Hw_t0_minus_segment_t0_qual_1_" + iChTag.str();  // Phase2 prim t minus segment t0 for qual 1
              m_plots[hName]->Fill(ph2TpgPhiHw_t0->at(iBestTpgPhiHw[iMB - 1]) - seg_phi_t0->at(iBestSeg[iMB - 1]));
            }

            hName = "ph2TpgPhiHw_quality_vs_seg_posGlb_phi_" + iChTag.str();  // quality vs segment posLoc_phi
            m_plots[hName]->Fill(seg_posGlb_phi->at(iBestSeg[iMB - 1]),
                                 ph2TpgPhiHw_quality->at(iBestTpgPhiHw[iMB - 1]));
          }
        }
        ...
        if (ph2TpgPhiHw_sector->at(itr) == iSec && ph2TpgPhiHw_wheel->at(itr) == iWh) {  // only wheel 2 and sector 12
          ...
          if (m_ph2TpgPhiHw && m_twinMux) {  // Madrid and TwinMux
            ...
            for (UInt_t itr2 = 0; itr2 < ltTwinMuxOut_nTrigs; ++itr2) {  //
              ...
              if (ltTwinMuxOut_sector->at(itr2) == iSec && ltTwinMuxOut_wheel->at(itr2) == iWh &&
                  ph2TpgPhiHw_station->at(itr) == ltTwinMuxOut_station->at(itr2)) {  // same wheel, sec, st
                ...
                hName =
                    "Phase2Hw_minus_TwinMuxOut_BX_vs_TwinMuxOut_phi_" + iChTag.str();  // Delta primitive BX vs TMux phi
                m_plots[hName]->Fill(phi_Ph1_conv * ltTwinMuxOut_phi->at(itr2),
                                     ph2TpgPhiHw_BX->at(itr) - ltTwinMuxOut_BX->at(itr2));

                hName = "Phase2Hw_minus_TwinMuxOut_quality_vs_TwinMuxOut_quality_" +
                        iChTag.str();  // Delta primitive quality vs TMux quality
                m_plots[hName]->Fill(ltTwinMuxOut_quality->at(itr2),
                                     ph2TpgPhiHw_quality->at(itr) - ltTwinMuxOut_quality->at(itr2));

                hName = "Phase2Hw_vs_TwinMuxOut_phi_" + iChTag.str();  // phi
                m_plots[hName]->Fill(phi_Ph1_conv * ltTwinMuxOut_phi->at(itr2),
                                     phi_Ph2_conv * ph2TpgPhiHw_phi->at(itr));

                hName = "Phase2Hw_vs_TwinMuxOut_phiB_" + iChTag.str();  // phiB
                m_plots[hName]->Fill(phiB_Ph1_conv * ltTwinMuxOut_phiB->at(itr2),
          ...
          if (m_ph2TpgPhiHw && m_ph2TpgPhiEmuAm) {  // Madrid and Emu Madrid
            ...
            for (UInt_t itr2 = 0; itr2 < ph2TpgPhiEmuAm_nTrigs; ++itr2) {  //
              ...
              if (ph2TpgPhiEmuAm_sector->at(itr2) == iSec && ph2TpgPhiEmuAm_wheel->at(itr2) == iWh &&
                  ph2TpgPhiHw_station->at(itr) == ph2TpgPhiEmuAm_station->at(itr2)) {  // same wheel, sec, st
                ...
                m_plots[hName]->Fill(phi_Ph2_conv * ph2TpgPhiEmuAm_phi->at(itr2),
                                     phi_Ph2_conv * ph2TpgPhiHw_phi->at(itr));
          ...
          if (m_ph2TpgPhiHw && m_ph2TpgPhiEmuHb) {  // Madrid and Pozzobon
            ...
            for (UInt_t itr2 = 0; itr2 < ph2TpgPhiEmuHb_nTrigs; ++itr2) {  //
              ...
              if (ph2TpgPhiEmuHb_sector->at(itr2) == iSec && ph2TpgPhiEmuHb_wheel->at(itr2) == iWh &&
                  ph2TpgPhiHw_station->at(itr) == ph2TpgPhiEmuHb_station->at(itr2)) {  // same wheel, sec, st
                ...
                m_plots[hName]->Fill(phi_Ph2_conv * ph2TpgPhiEmuHb_phi->at(itr2),
                                     phi_Ph2_conv * ph2TpgPhiHw_phi->at(itr));
    ...
    if (m_ph2TpgPhiEmuHb) {  // Emu Madrid
      ...
      for (UInt_t itr = 0; itr < ph2TpgPhiEmuHb_nTrigs; ++itr) {
        if (ph2TpgPhiEmuHb_sector->at(itr) == iSec &&
            ph2TpgPhiEmuHb_wheel->at(itr) == iWh) {  // only wheel 2 and sector 12
    ...
    if (m_ph2TpgPhiEmuAm) {
      for (UInt_t itr = 0; itr < ph2TpgPhiEmuAm_nTrigs; ++itr) {
        if (ph2TpgPhiEmuAm_sector->at(itr) == iSec &&
            ph2TpgPhiEmuAm_wheel->at(itr) == iWh) {  // only wheel 2 and sector 12
...
UInt_t DTNtupleTriggerAnalyzer::getBestSegm(const Int_t stMu, const Int_t secMu, const Int_t whMu) {
  UInt_t iBest = 9999;
  Int_t nhits = 0;
  Int_t nmin_phihits = 4;

  for (UInt_t iSeg = 0; iSeg < seg_nSegments; ++iSeg) {  // request on the tdc segment
    if (seg_wheel->at(iSeg) == whMu && seg_sector->at(iSeg) == secMu && seg_station->at(iSeg) == stMu &&
        seg_hasPhi->at(iSeg) > 0 && seg_phi_nHits->at(iSeg) >= nmin_phihits && nhits <= seg_phi_nHits->at(iSeg)) {
...
UInt_t DTNtupleTriggerAnalyzer::getBestTwinMuxOut(const Int_t stMu, const Int_t secMu, const Int_t whMu) {
  ...
  for (UInt_t iTwinMuxOut = 0; iTwinMuxOut < ltTwinMuxOut_nTrigs; ++iTwinMuxOut) {
    ...
    if (stTwinMuxOut == 4 && secTwinMuxOut == 13) {
      secTwinMuxOut = 4;
    } else if (stTwinMuxOut == 4 && secTwinMuxOut == 14) {
      secTwinMuxOut = 10;
    }

    if (whMu == whTwinMuxOut && secMu == secTwinMuxOut && stMu == stTwinMuxOut &&
        //	 bxTwinMuxOut == 0       &&
...
UInt_t DTNtupleTriggerAnalyzer::getBestTwinMuxOutBXOK(const Int_t stMu, const Int_t secMu, const Int_t whMu) {
  UInt_t ibestTwinMuxOut = 9999;
  Int_t bestTwinMuxOutQual = -1;
  ...
  for (UInt_t iTwinMuxOut = 0; iTwinMuxOut < ltTwinMuxOut_nTrigs; ++iTwinMuxOut) {
    ...
    if (whMu == whTwinMuxOut && secMu == secTwinMuxOut && stMu == stTwinMuxOut && bxTwinMuxOut == 0 &&
        qualityTwinMuxOut > bestTwinMuxOutQual)
...
UInt_t DTNtupleTriggerAnalyzer::getBestTpgPhiHw(const Int_t stMu, const Int_t secMu, const Int_t whMu) {
  ...
  for (UInt_t iTpgPhiHw = 0; iTpgPhiHw < ph2TpgPhiHw_nTrigs; ++iTpgPhiHw) {
    ...
    if (whMu == whTpgPhiHw && secMu == secTpgPhiHw && stMu == stTpgPhiHw &&
        //	 bxTpgPhiHw == 0       &&
...
UInt_t DTNtupleTriggerAnalyzer::getBestTpgPhiHwBXOK(const Int_t stMu, const Int_t secMu, const Int_t whMu) {
  UInt_t ibestTpgPhiHw = 9999;
  Int_t bestTpgPhiHwQual = -1;
  ...
  for (UInt_t iTpgPhiHw = 0; iTpgPhiHw < ph2TpgPhiHw_nTrigs; ++iTpgPhiHw) {
    ...
    if (whMu == whTpgPhiHw && secMu == secTpgPhiHw && stMu == stTpgPhiHw && bxTpgPhiHw == 0 &&  // -269
        qualityTpgPhiHw > bestTpgPhiHwQual)
...
UInt_t DTNtupleTriggerAnalyzer::getBestTpgPhiEmuAm(const Int_t stMu, const Int_t secMu, const Int_t whMu) {
  ...
  for (UInt_t iTpgPhiEmuAm = 0; iTpgPhiEmuAm < ph2TpgPhiEmuAm_nTrigs; ++iTpgPhiEmuAm) {
    ...
    if (whMu == whTpgPhiEmuAm && secMu == secTpgPhiEmuAm && stMu == stTpgPhiEmuAm &&
        //	 bxTpgPhiEmuAm == 0       &&
...
UInt_t DTNtupleTriggerAnalyzer::getBestTpgPhiEmuAmBXOK(const Int_t stMu, const Int_t secMu, const Int_t whMu) {
  UInt_t ibestTpgPhiEmuAm = 9999;
  Int_t bestTpgPhiEmuAmQual = -1;
  ...
  for (UInt_t iTpgPhiEmuAm = 0; iTpgPhiEmuAm < ph2TpgPhiEmuAm_nTrigs; ++iTpgPhiEmuAm) {
    ...
    if (whMu == whTpgPhiEmuAm && secMu == secTpgPhiEmuAm && stMu == stTpgPhiEmuAm && bxTpgPhiEmuAm == 0 &&
        qualityTpgPhiEmuAm > bestTpgPhiEmuAmQual)
...
UInt_t DTNtupleTriggerAnalyzer::getBestTpgPhiEmuHb(const Int_t stMu, const Int_t secMu, const Int_t whMu) {
  ...
  for (UInt_t iTpgPhiEmuHb = 0; iTpgPhiEmuHb < ph2TpgPhiEmuHb_nTrigs; ++iTpgPhiEmuHb) {
    ...
    if (whMu == whTpgPhiEmuHb && secMu == secTpgPhiEmuHb && stMu == stTpgPhiEmuHb &&
        //	 bxTpgPhiEmuHb == 0       &&
